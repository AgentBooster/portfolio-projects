...
def main():
    ...
    model = TFBertForMaskedLM.from_pretrained(MODEL, use_safetensors=False)
...
def get_color_for_attention_score(attention_score):
    ...
    """
    score = max(0, min(1, attention_score))
    value = int(score * 255)
...
def visualize_attentions(tokens, attentions):
    ...
    for layer_index, layer_attention in enumerate(attentions):
        for head_index in range(layer_attention.shape[1]):
            attention_weights = layer_attention[0][head_index].numpy()
            generate_diagram(
                layer_index + 1,
                head_index + 1,
                tokens,
