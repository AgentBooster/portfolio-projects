(function () {
  ...
  if (window.FisiaWidgetLoaded) return;
  window.FisiaWidgetLoaded = true;

  const DEFAULT_CONFIG = {
    company_name: 'Fisia',
    welcome_message: 'Hola, somos Fisia. ¿En qué podemos ayudarte hoy?',
    logo_url: 'https://res.cloudinary.com/dsdnpstgi/image/upload/v1762990590/ChatGPT_Image_12_nov_2025_08_36_04_p.m._cublmj.png',
  ...
  const styles = `
    ...
    #fisia-widget-button:hover {
      transform: scale(1.08);
      background: rgba(14, 116, 144, 0.9);
    }
    #fisia-widget-button svg {
      width: 24px;
      height: 24px;
      position: absolute;
      transition: opacity 0.3s ease, transform 0.3s ease;
    }
    #fisia-widget-button .chat-icon {
      opacity: 1;
      transform: rotate(0deg) scale(1);
    }
    #fisia-widget-button .close-icon {
      opacity: 0;
      transform: rotate(90deg) scale(0.5);
    }
    #fisia-widget-button.open .chat-icon {
      opacity: 0;
      transform: rotate(-90deg) scale(0.5);
    }
    #fisia-widget-button.open .close-icon {
      opacity: 1;
      transform: rotate(0deg) scale(1);
    }
    #fisia-widget-panel {
    ...
    @keyframes fisiaPopIn {
      from { opacity: 0; transform: scale(0.94); }
      to { opacity: 1; transform: scale(1); }
    }
    @keyframes fisia-typing {
    ...
    .fisia-welcome-msg {
      ...
      padding: 20px 24px;
      display: none;
      animation: fisiaPopIn 0.2s ease-out;
      font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;
    }
    .fisia-welcome-msg-close {
      position: absolute;
      top: 14px;
      right: 14px;
      width: 26px;
    ...
    .fisia-welcome-msg-close:hover {
      background: rgba(14, 116, 144, 0.16);
      color: #0f172a;
      transform: scale(1.06);
    }
    .fisia-welcome-msg-title {
      color: #1f2937;
      font-weight: 700;
      margin: 0 32px 8px 0;
      font-size: 17px;
    }
    .fisia-welcome-msg-text {
      color: #334155;
      font-size: 14.5px;
      line-height: 1.55;
      margin: 0;
  ...
  const icons = {
    ...
    logo: '<defs><linearGradient id="fisia-g" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="#38bdf8"/><stop offset="100%" stop-color="#0ea5e9"/></linearGradient></defs><path d="M15 120 L60 40 L85 75 L110 55 L150 120 Z" fill="url(#fisia-g)"/><path d="M60 40 L85 75 L110 55" fill="none" stroke="#e2e8f0" stroke-width="6" stroke-linecap="round"/>',
  ...
  function getUserId() {
    let id = localStorage.getItem('fisia_chatwidget_user_uuid');
    if (!id) {
      id = crypto.randomUUID();
      localStorage.setItem('fisia_chatwidget_user_uuid', id);
    }
    return id;
  }
  ...
  function updateTooltip() {
    let tooltip = document.getElementById('fisia-tooltip');
    if (state.tooltip.visible) {
      if (!tooltip) {
        tooltip = document.createElement('div');
        tooltip.id = 'fisia-tooltip';
        tooltip.className = 'fisia-tooltip';
  ...
  function renderHomeView() {
    ...
    const companyName = state.config?.company_name || 'Fisia';
    ...
    if (logoUrl) {
      ...
      const logoIcon = createIcon('logo', 'fisia-logo');
    ...
    Object.values(agentProfiles).forEach((profile) => {
      if (profile?.avatar) {
        ...
        img.className = 'fisia-agent-avatar';
  ...
  function renderTypingIndicator(agentName) {
    ...
    for (let i = 0; i < 3; i += 1) {
      ...
      dot.style.cssText = `width:8px;height:8px;border-radius:50%;background:rgba(71,85,105,0.65);animation: fisia-typing ${1 + i * 0.1}s infinite;`;
  ...
  function renderChatView() {
    ...
    agentInfo.innerHTML = `<div style="font-weight:700">${state.agent?.name === 'Fisia' ? 'Chatea con nosotros' : state.agent?.name || 'Fisia'}</div><div style="font-size:12px;color:rgba(255,255,255,0.82)">Online</div>`;
    ...
    charCount.style.cssText = 'position:absolute;bottom:10px;left:22px;font-size:11px;font-weight:600;color:#f87171;display:none;';
    form.appendChild(charCount);

    const actions = document.createElement('div');
    actions.style.cssText = 'position:absolute;bottom:8px;right:18px;display:flex;align-items:center;gap:8px;';
    ...
    setTimeout(() => {
      const messagesEl = document.getElementById('fisia-chat-messages');
      if (messagesEl) {
  ...
  function renderSearchView({ title, iconName, placeholder, itemType }) {
    ...
    searchInput.style.cssText = 'width:100%;padding:10px 14px 10px 36px;border-radius:12px;border:1px solid rgba(255,255,255,0.25);background:rgba(255,255,255,0.08);color:white;font-size:14px;outline:none;box-sizing:border-box;';
  ...
  function renderFileUploadView() {
    ...
    hiddenInput.id = 'fisia-file-input';
    ...
    dropArea.setAttribute('for', 'fisia-file-input');
    ...
    addFileLabel.setAttribute('for', 'fisia-file-input');
    ...
    const textArea = document.createElement('textarea');
    textArea.style.cssText = 'width:100%;border-radius:12px;border:1px solid rgba(255,255,255,0.2);background:rgba(255,255,255,0.12);color:white;font-size:14px;padding:12px;resize:none;min-height:80px;box-sizing:border-box;';
  ...
  function renderContent() {
    ...
    if (isMobileViewport() && state.isOpen) {
      ...
      mobileClose.className = 'fisia-mobile-close';
  ...
  function toggleChat() {
    ...
    const panel = document.getElementById('fisia-widget-panel');
    const button = document.getElementById('fisia-widget-button');
    const welcomeMsg = document.getElementById('fisia-welcome-msg');
  ...
  function initializeChat() {
    ...
    setTimeout(() => {
      if (state.messages.length === 0) {
        ...
        setTimeout(() => {
          state.isTyping = false;
          const welcomeText = `¡Hola! Somos ${state.config?.company_name || 'Fisia'}. Deja tu consulta y te respondemos a la brevedad.`;
          const botTimestamp = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
          state.messages.push({
            id: Date.now(),
            text: welcomeText,
  ...
  async function init() {
    ...
    const widgetContainer = document.getElementById('fisia-widget');
    if (!widgetContainer) {
      const autoContainer = document.createElement('div');
      autoContainer.id = 'fisia-widget';
    ...
    state.agent = { name: state.config.company_name || 'Fisia', avatar: null };

    const button = document.createElement('button');
    button.id = 'fisia-widget-button';
    ...
    panel.id = 'fisia-widget-panel';
    panel.style.display = 'none';

    const welcomeMsg = document.createElement('div');
    welcomeMsg.id = 'fisia-welcome-msg';
    welcomeMsg.className = 'fisia-welcome-msg';

    const closeWelcomeBtn = document.createElement('button');
    closeWelcomeBtn.className = 'fisia-welcome-msg-close';
    ...
    welcomeContent.innerHTML = `
      <p class="fisia-welcome-msg-title">¡Hola! 👋</p>
      <p class="fisia-welcome-msg-text">Aquí puedes escribirnos. ¿Hay algo en lo que podamos ayudarte?</p>
    `;
