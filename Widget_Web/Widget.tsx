...
const Icon = ({ name, className = '' }: { name: string; className?: string }) => {
  const icons: Record<string, JSX.Element> = {
    ...
    logo: <>
      <defs><linearGradient id="g-react" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stopColor="#38bdf8"/><stop offset="100%" stopColor="#0ea5e9"/></linearGradient></defs>
  ...
  return (
    <svg className={`${size} ${className}`} viewBox={viewBox} fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
      {icons[name]}
    </svg>
  );
};
...
const SimpleMarkdown = ({ text }: { text: string }) => {
  ...
  return <div className="leading-snug whitespace-pre-wrap break-words" dangerouslySetInnerHTML={htmlContent}></div>;
};
    ...
    <div className="relative z-10 h-full overflow-y-auto">
      <header className="p-4 flex items-center justify-between text-slate-100">
        <div className="flex items-center gap-2">
          <div className="w-10 h-10"><Icon name="logo" /></div>
          <span className="font-bold text-lg">Altavia</span>
        </div>
        <div className="flex items-center -space-x-2">
          {Object.values(agentProfiles).map((profile: { name: string; avatar: string }) => (
            <img 
              key={profile.name} 
              src={profile.avatar} 
              alt={profile.name}
      ...
      <div className="p-4 space-y-4">
        <div onClick={() => setActiveView('chat')} className="bg-white/20 backdrop-blur-md rounded-xl shadow-lg border border-white/10 p-4 flex items-center justify-between cursor-pointer hover:bg-white/30 hover:-translate-y-0.5 transform transition-all duration-200">
            <div>
        ...
        <div className="bg-white/20 backdrop-blur-md rounded-xl shadow-lg border border-white/10 p-4">
            ...
            <div className="mt-3 border-t border-white/20 -mx-4"></div>
            <div className="mt-3 space-y-1">
               <div onClick={() => setActiveView('faq')} className="flex justify-between items-center cursor-pointer group p-2 -m-2 rounded-lg hover:bg-white/10 transition-colors">
                  <span className="text-slate-100 font-medium">Preguntas</span>
                  <Icon name="chevronRight" className="w-5 h-5 text-slate-200 group-hover:translate-x-1 transition-transform" />
               </div>
               <div onClick={() => setActiveView('articles')} className="flex justify-between items-center cursor-pointer group p-2 -m-2 rounded-lg hover:bg-white/10 transition-colors">
...
const CommonSearchView = ({ title, iconName, placeholder, itemType, setActiveView }: { title: string; iconName: string; placeholder: string; itemType: string; setActiveView: (view: string) => void }) => {
    const [searchTerm, setSearchTerm] = useState('');
    const hasResults = false;
    const messageContent = searchTerm.trim() === ''
    ...
    return (
        <div className="h-full flex flex-col bg-transparent">
            <header className="bg-black/20 text-white p-4 flex items-center shrink-0">
                <button onClick={() => setActiveView('home')} className="rounded-full w-8 h-8 inline-flex items-center justify-center hover:bg-white/20 focus:outline-none" aria-label="Volver a inicio">
            ...
            <div className="p-4 bg-black/10 border-b border-white/10 shrink-0">
                <div className="relative">
                    <input type="text" placeholder={placeholder} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-white/20 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-sky-400 bg-white/10 text-white placeholder:text-slate-300" />
                    <div className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300"><Icon name="search" className="w-5 h-5" /></div>
                </div>
            </div>
            <div className="flex-1 flex flex-col items-center justify-center text-center p-4 bg-transparent">
                ...
                <p className="text-lg font-semibold text-slate-100">No hay resultados</p>
                <p className="text-sm text-slate-300 max-w-xs mt-1">{messageContent}</p>
                <button onClick={() => setActiveView('chat')} className="mt-6 bg-sky-600/50 text-white font-bold py-3 px-6 rounded-lg hover:bg-sky-600/70 border border-white/20 transition-colors">Chatear ahora</button>
            </div>
        </div>
    );
};
...
const TypingIndicator = ({ agentName }: { agentName: string }) => (
    <div className="flex flex-col w-full items-start">
        ...
        <div className="max-w-[85%] p-3 text-sm shadow-sm bg-white/30 text-slate-100 border border-white/20 rounded-t-2xl rounded-br-2xl flex items-center gap-2">
            <div className="w-2 h-2 bg-slate-200 rounded-full animate-[bounce_1s_infinite_0s]"></div>
            <div className="w-2 h-2 bg-slate-200 rounded-full animate-[bounce_1s_infinite_.2s]"></div>
            <div className="w-2 h-2 bg-slate-200 rounded-full animate-[bounce_1s_infinite_.4s]"></div>
        </div>
    </div>
);

const ChatView = ({ messages, onSendMessage, setActiveView, isMuted, toggleMute, isTyping, agent, isAgentAssigned, isBotReplying, onShowTooltip, onHideTooltip }: { messages: any[]; onSendMessage: (text: string) => void; setActiveView: (view: string) => void; isMuted: boolean; toggleMute: () => void; isTyping: boolean; agent: { name: string; avatar: string | null }; isAgentAssigned: boolean; isBotReplying: boolean; onShowTooltip: (e: React.MouseEvent, text: string) => void; onHideTooltip: () => void }) => {
  ...
  useEffect(() => { if (chatMessagesRef.current) { chatMessagesRef.current.scrollTop = chatMessagesRef.current.scrollHeight; } }, [messages, isTyping]);
  useEffect(() => {
    if (!('webkitSpeechRecognition' in window)) { console.error("Web Speech API no es compatible."); return; }
    ...
    recognition.onresult = (event) => {
      ...
      if (finalTranscript) { setInputValue(prevVal => [prevVal, finalTranscript.trim()].filter(Boolean).join(' ')); }
    };
    recognition.onerror = (event) => { console.error("Error en reconocimiento de voz:", event.error); setIsRecording(false); };
  ...
  useEffect(() => {
    ...
    if (textarea) {
      textarea.style.height = 'auto'; const maxHeight = 120;
      textarea.style.height = `${Math.min(textarea.scrollHeight, maxHeight)}px`;
    }
  }, [inputValue]);

  const handleInputChange = (e) => setInputValue(e.target.value);
  const handleSubmit = (e) => { e.preventDefault(); onSendMessage(inputValue); setInputValue(''); };
  const toggleRecording = () => {
    if (!recognitionRef.current) return;
    if (isRecording) { recognitionRef.current.stop(); } else { try { recognitionRef.current.start(); setIsRecording(true); } catch (err) { console.error("Error al iniciar reconocimiento:", err); } }
  };
    ...
    <div className="h-full flex flex-col bg-transparent">
       <header className="bg-black/20 text-white p-4 flex items-center justify-between shrink-0 relative">
          <div className="flex items-center gap-3">
              <button onClick={() => setActiveView('home')} className="rounded-full w-8 h-8 inline-flex items-center justify-center hover:bg-white/20 focus:outline-none" aria-label="Volver a inicio"><Icon name="chevronRight" className="w-5 h-5 rotate-180"/></button>
              <div className="w-10 h-10 flex items-center justify-center">
                 {agent.avatar ? <img src={agent.avatar} alt={agent.name} className="w-full h-full rounded-full object-cover border-2 border-white/50" draggable="false" onDragStart={(e) => e.preventDefault()} onContextMenu={(e) => e.preventDefault()} /> : <Icon name="logo"/>}
              </div>
              <div>
           ...
           <div className="flex items-center gap-1">
              <button type="button" onClick={toggleMute} disabled={!isAgentAssigned} className="w-8 h-8 rounded-full grid place-items-center hover:bg-white/20 text-white disabled:opacity-50 disabled:cursor-not-allowed" aria-label="Silenciar/Activar sonido">
                  <Icon name={isMuted ? 'volume-off' : 'volume'} className="w-5 h-5" />
              </button>
              <button type="button" onClick={toggleRecording} disabled={!isAgentAssigned} className={`w-8 h-8 rounded-full grid place-items-center transition-colors ${isRecording ? 'bg-red-500' : 'hover:bg-white/20'} text-white disabled:opacity-50 disabled:cursor-not-allowed`} aria-label="Iniciar/Detener grabación de voz">
          ...
          {messages.map((msg, index) => {
            ...
            return (
              <div key={msg.id} className={`flex flex-col w-full ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}>
                ...
                <div className={`max-w-[85%] p-3 text-sm shadow-sm ${ msg.sender === 'user' ? 'bg-sky-600/90 text-white rounded-t-2xl rounded-bl-2xl' : 'bg-white/30 text-slate-100 border border-white/20 rounded-t-2xl rounded-br-2xl'}`}>
                  <SimpleMarkdown text={msg.text} />
                  <span className={`text-xs mt-1 block text-right ${ msg.sender === 'user' ? 'text-white/70' : 'text-slate-300'}`}>{msg.timestamp}</span>
                </div>
              </div>
            );
          })}
      ...
      <form onSubmit={handleSubmit} className="bg-black/20 border-t border-white/20 relative shrink-0" style={{ minHeight: '120px' }}>
        <textarea ref={textareaRef} value={inputValue} onChange={handleInputChange} onKeyDown={(e) => { if(e.key === 'Enter' && !e.shiftKey) handleSubmit(e);}} maxLength={MAX_CHAT_CHAR} className="absolute inset-0 w-full h-full p-4 pr-16 resize-none bg-transparent text-white placeholder:text-slate-300 focus:outline-none disabled:bg-black/20 disabled:cursor-not-allowed" style={{ fontSize: '16px' }} rows={1} placeholder={isAgentAssigned ? "Escribe un mensaje..." : "Espera a que un agente se conecte..."} disabled={isInputDisabled} />
        {inputValue.length >= MAX_CHAT_CHAR && (<div className="absolute bottom-3 left-4 text-xs font-semibold text-red-400">{inputValue.length} / {MAX_CHAT_CHAR}</div>)}
        <div className="absolute bottom-3 right-3 flex items-center">
            {inputValue.trim() ? <button type="submit" disabled={isInputDisabled} className="w-9 h-9 rounded-full bg-sky-600/80 hover:bg-sky-500/80 text-white grid place-items-center shrink-0 transition-all animate-[pop_.15s_ease-out] disabled:bg-sky-400/50 disabled:cursor-not-allowed"><Icon name="send" className="w-5 h-5"/></button> : 
            <div 
                className="relative"
                onMouseEnter={(e) => onShowTooltip(e, 'Desactivado')}
                onMouseLeave={onHideTooltip}
            >
                <button type="button" disabled={true} className="w-9 h-9 rounded-full bg-transparent text-slate-300 grid place-items-center shrink-0 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
...
const FileUploadView = ({ setActiveView, onSendFile }: { setActiveView: (view: string) => void; onSendFile: (file: File, text: string) => void }) => {
    ...
    const handleDrop = (e) => { e.preventDefault(); setIsDragging(false); if (e.dataTransfer.files && e.dataTransfer.files[0]) handleFileSelect(e.dataTransfer.files[0]); };
    const handleFileChange = (e) => { if (e.target.files && e.target.files[0]) handleFileSelect(e.target.files[0]); };
    const handleSend = () => { if (file) onSendFile(file, text); };

    return (
        <div className="h-full flex flex-col bg-transparent">
            <header className="bg-black/20 text-white p-4 flex items-center shrink-0">
                <button onClick={() => setActiveView('chat')} className="rounded-full w-8 h-8 inline-flex items-center justify-center hover:bg-white/20 focus:outline-none" aria-label="Volver al chat">
                    <Icon name="chevronRight" className="w-5 h-5 rotate-180" />
                </button>
            ...
            <div className="flex-1 p-4 flex flex-col gap-4 overflow-y-auto">
                <input id="file-upload-input" type="file" ref={fileInputRef} onChange={handleFileChange} className="hidden" accept=".jpg,.jpeg,.png,.pdf" />
                ...
                {!file ? (
                    <label 
                        ...
                        onDrop={handleDrop} 
                        className={`flex-1 flex flex-col items-center justify-center p-4 border-2 border-dashed rounded-lg cursor-pointer transition-colors ${isDragging ? 'bg-sky-500/20 border-sky-400' : 'bg-white/10'} ${error ? 'border-red-500' : 'border-white/20 hover:bg-white/20'}`}
                    >
                    ...
                    <>
                        ...
                        <div className="flex-shrink-0 flex items-center gap-2">
                            ...
                            <label htmlFor="file-upload-input" className="w-16 h-16 rounded-lg border bg-white/10 border-white/20 flex items-center justify-center cursor-pointer hover:bg-white/20 transition-colors">
                ...
                <div className="relative flex-shrink-0">
                    <textarea value={text} onChange={(e) => setText(e.target.value)} maxLength={MAX_TEXT_CHAR} placeholder="Añadir un mensaje (opcional)..." className="w-full p-3 pr-12 border border-white/20 rounded-lg text-sm bg-white/10 text-white placeholder:text-slate-300 focus:outline-none focus:ring-2 focus:ring-sky-400 resize-none shadow-sm" rows={3}></textarea>
            ...
            <footer className="p-4 bg-black/20 border-t border-white/10 shrink-0">
                <button onClick={handleSend} disabled={!file} className={`w-full font-bold py-3 px-6 rounded-lg transition-colors ${file ? 'bg-sky-600/80 hover:bg-sky-500/90 text-white border border-white/20' : 'bg-sky-400/30 text-slate-300 cursor-not-allowed'}`}>
                    Enviar
                </button>
            </footer>
        </div>
...
const ChatWidget = () => {
  ...
  const [agent, setAgent] = useState({ name: 'Altavia', avatar: null });
  ...
  const AGENT_PROFILES = {
    'Ara': { name: 'Ara', avatar: 'https://res.cloudinary.com/dsdnpstgi/image/upload/v1760978463/image_kjvbyu.jpg' },
    'Eve': { name: 'Eve', avatar: 'https://res.cloudinary.com/dsdnpstgi/image/upload/v1760978711/e661445e-0368-456b-af4e-04fe7df03f3b_qd3a8r.jpg' }
  };

  useEffect(() => {
    let id = localStorage.getItem('chatwidget_user_uuid');
    if (!id) { id = crypto.randomUUID(); localStorage.setItem('chatwidget_user_uuid', id); }
    setUserId(id);

    let agentName = localStorage.getItem('chatwidget_assigned_agent');
  ...
  const handleShowTooltip = (e, text) => {
  ...
  const processReplyQueue = async (queue) => {
      ...
      for (const messageText of queue) {
          ...
          const botTimestamp = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
          setMessages(prev => [...prev, { id: Date.now() + Math.random(), text: messageText, sender: 'bot', timestamp: botTimestamp }]);
          if (!isMuted) playNotificationSound();
      }
  ...
  const handleSendMessage = (text) => {
    ...
    const newUserMessage = { id: Date.now(), text: trimmedText, sender: 'user', timestamp };
  ...
  const handleSendFile = async (file, text) => {
    ...
    setMessages(prev => [ ...prev, { id: Date.now(), text: fileMessageText, sender: 'user', timestamp }]);
    setActiveView('chat');
    ...
    try {
        ...
        const payload = {
            ...
            content_type: 'file',
            text_content: text || '',
            file_data: { file_name: file.name, file_type: file.type, file_base64: base64Content }
        };
        await postToWebhook(payload);
    } catch (error) {
        console.error("Error al convertir archivo a Base64:", error);
        const errorTimestamp = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        setMessages(prev => [...prev, { id: Date.now() + 1, text: 'Hubo un error al procesar tu archivo. Inténtalo de nuevo.', sender: 'bot', timestamp: errorTimestamp }]);
    }
  };
  ...
  const renderContent = () => {
    ...
    switch(activeView) {
      case 'home': return <HomeView setActiveView={setActiveView} agentProfiles={AGENT_PROFILES} />;
      case 'chat': return <ChatView messages={messages} onSendMessage={handleSendMessage} setActiveView={setActiveView} isMuted={isMuted} toggleMute={toggleMute} isTyping={isTyping} agent={agent} isAgentAssigned={isAgentAssigned} isBotReplying={isBotReplying} onShowTooltip={handleShowTooltip} onHideTooltip={handleHideTooltip} />;
      case 'fileUpload': return <FileUploadView setActiveView={setActiveView} onSendFile={handleSendFile} />;
      case 'faq': return <FAQView setActiveView={setActiveView} />;
      case 'articles': return <ArticlesView setActiveView={setActiveView} />;
      default: return <HomeView setActiveView={setActiveView} agentProfiles={AGENT_PROFILES} />;
    }
  };
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 font-sans flex flex-col items-end">
      ...
      {isOpen && (
        <div className={`mb-4 overflow-hidden shadow-2xl border border-white/20 bg-black/30 backdrop-blur-2xl flex flex-col animate-[pop_.15s_ease-out] ${
          isMobile 
          ...
          {activeView !== 'chat' && activeView !== 'fileUpload' && (
            <footer className="bg-black/20 border-t border-white/10 grid grid-cols-4 text-xs text-slate-300 shrink-0">
              <button onClick={() => setActiveView('home')} className={`py-2 flex flex-col items-center gap-1 transition-colors ${activeView === 'home' ? 'text-white font-semibold' : 'hover:text-sky-300'}`}><Icon name="home" className="w-5 h-5"/><span>Inicio</span></button>
              <button onClick={() => setActiveView('chat')} className={`py-2 flex flex-col items-center gap-1 transition-colors ${activeView === 'chat' ? 'text-white font-semibold' : 'hover:text-sky-300'}`}><Icon name="chat" className="w-5 h-5"/><span>Conversación</span></button>
              <button onClick={() => setActiveView('faq')} className={`py-2 flex flex-col items-center gap-1 transition-colors ${activeView === 'faq' ? 'text-white font-semibold' : 'hover:text-sky-300'}`}><Icon name="faq" className="w-5 h-5"/><span>Preguntas</span></button>
              <button onClick={() => setActiveView('articles')} className={`py-2 flex flex-col items-center gap-1 transition-colors ${activeView === 'articles' ? 'text-white font-semibold' : 'hover:text-sky-300'}`}><Icon name="articles" className="w-5 h-5"/><span>Artículos</span></button>
            </footer>
          )}
        </div>
      )}
      ...
      {(!isMobile || !isOpen) && (
        <button onClick={toggleChat} type="button" className="w-14 h-14 rounded-full shadow-lg bg-sky-500/50 hover:bg-sky-500/80 backdrop-blur-md border border-white/20 text-white grid place-items-center transition-transform hover:scale-110 focus:outline-none" aria-expanded={isOpen}>
          ...
          <div className="relative w-6 h-6">
            <Icon name="chat" className={`absolute transition-all duration-300 ${isOpen ? 'opacity-0 -rotate-90 scale-50' : 'opacity-100 rotate-0 scale-100'}`} />
            <Icon name="close" className={`absolute transition-all duration-300 ${isOpen ? 'opacity-100 rotate-0 scale-100' : 'opacity-0 rotate-90 scale-50'}`} />
          </div>
        </button>
      )}
    </div>
...
export default function App() {
  return (
    <div className="w-full h-screen bg-cover bg-center" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1554034483-04fda0d3507b?q=80&w=2670&auto=format&fit=crop')" }}>
      <ChatWidget />
    </div>
  );
}
